...
import com.example.uxcamptest.databinding.ActivityMainBinding
import com.uxcamp.analytics.AnalyticsSDK
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Initialize sdk and start analytics session
       AnalyticsSDK.Builder().chukkerEnable(false).apiKey("").appVersion("1.0.1").environment(ENVIRONMENT.DEBUG).build().start(this)
        //Add Applaunched event as sample
        val event = Event("AppLaunched", mapOf("action" to "launched", "label" to "launched app"))
        ...
        binding.fab.setOnClickListener { view ->
            val event = Event("UserAction", mapOf("action" to "button_click", "label" to "fabicon first"))
            AnalyticsSingleton.addEvent(event)
